...
|------------|----------|----------------------|
| `DB_HOST` | Адрес сервера базы данных | `checkpoint.devman.org` |
| `DB_PORT` | Порт для подключения | `5434` |
| `DB_NAME` | Имя базы данных | `checkpoint` |
| `DB_USER` | Имя пользователя БД | `guard` |
| `DB_PASSWORD` | Пароль пользователя БД | `osim5` |
| `ALLOWED_HOSTS` | Список хостов, которым разрешено обслуживать приложение (важно для безопасности) | `ALLOWED_HOSTS` теперь имеет безопасное значение по умолчанию `[ ]`, а для продакшена её нужно задавать явно|
