...
Проект использует переменные окружения для конфигурации. Все настройки хранятся в файле `.env` в корне проекта.
...
- **Пример:** `django-insecure-your-secret-key-here`
- **По умолчанию:** Используется значение из settings.py
- **Важно:** В продакшн-среде всегда должен быть задан через .env
